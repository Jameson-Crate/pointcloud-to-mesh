...
def compute_barycentric_coordinates(p, a, b, c):
    ...
    Returns:
        numpy.ndarray: Barycentric coordinates (u, v, w)
    """
    v0 = b - a
    v1 = c - a
    v2 = p - a
...
def is_point_in_triangle(barycentric):
    """
    Check if a point is inside a triangle using its barycentric coordinates.
    
    Args:
        barycentric: Barycentric coordinates (u, v, w)
        
    Returns:
        bool: True if point is inside triangle, False otherwise
    """
    return all(0 <= c <= 1 for c in barycentric) and abs(sum(barycentric) - 1.0) < 1e-10


def map_texture(pcd, mesh, max_distance=None, method='nearest'):
    """
    Map texture (color) from point cloud to mesh.
    
    Args:
        pcd: Point cloud with color information
        mesh: Mesh to apply texture to
        max_distance: Maximum distance to consider for point-to-triangle mapping
        method: Method for texture mapping ('nearest' or 'weighted')
    ...
    vertex_weights = np.zeros(len(mesh_vertices))
    ...
    if max_distance is None:
        ...
        max_distance = mesh_extent * 0.01  # Use 1% of the bounding box size as default
    
    # Process each triangle
    print(f"Mapping colors using {method} method (max distance: {max_distance:.6f})")
    for i, triangle in tqdm(enumerate(mesh_triangles), total=len(mesh_triangles), desc="Processing triangles"):
        v0, v1, v2 = mesh_vertices[triangle]
        
        # Calculate triangle centroid
        centroid = (v0 + v1 + v2) / 3.0
        
        # Find nearby points in the point cloud
        indices = pcd_tree.query_ball_point(centroid, max_distance)
        
        if not indices:
            continue
        
        # For each nearby point, compute the color contribution
        for idx in indices:
            point = cloud_points[idx]
            color = cloud_colors[idx]
            
            # Compute barycentric coordinates
            bary = compute_barycentric_coordinates(point, v0, v1, v2)
            
            if method == 'nearest':
                # Just use the nearest point's color for all vertices
                for j, vertex_idx in enumerate(triangle):
                    vertex_colors[vertex_idx] += color
                    vertex_weights[vertex_idx] += 1
            
            elif method == 'weighted':
                # Use barycentric coordinates to weight the color contribution
                if is_point_in_triangle(bary):
                    for j, vertex_idx in enumerate(triangle):
                        weight = bary[j]
                        vertex_colors[vertex_idx] += color * weight
                        vertex_weights[vertex_idx] += weight
    
    # Normalize vertex colors
    for i in range(len(vertex_colors)):
        if vertex_weights[i] > 0:
            vertex_colors[i] /= vertex_weights[i]
    ...
    colored_mesh.triangles = o3d.utility.Vector3iVector(mesh_triangles)
...
def alternative_texture_mapping(pcd, mesh, max_distance=None, num_neighbors=3):
    """
    Alternative method to map textures using interpolation of nearest points.
...
def main():
    ...
    parser.add_argument('--method', choices=['nearest', 'weighted', 'alternative'], default='alternative',
                        help='Method for texture mapping')
    ...
    parser.add_argument('--neighbors', type=int, default=3,
                        help='Number of neighbors for alternative method')
    parser.add_argument('--visualize', action='store_true',
                        help='Visualize the textured mesh')
    ...
    try:
        ...
        if args.method == 'alternative':
            colored_mesh = alternative_texture_mapping(
                pcd, mesh, args.max_distance, args.neighbors
            )
        else:
            colored_mesh = map_texture(
                pcd, mesh, args.max_distance, args.method
            )
        ...
        if args.fix_rotation and file_ext.lower() in ['.obj', '.glb', '.gltf']:
        ...
        if not success:
            ...
            if file_ext != '.ply':
                ...
                if fallback_success:
                    ...
                    print(f"Failed to save fallback file as well.")
            return 1
        
        # Visualize if requested
        if args.visualize:
            print("Visualizing textured mesh...")
